...
# --- FINAL, COMPLETE & WORKING VERSION with New Optimizer Strategy ---
...
# --- Argument Parsing ---
def get_args():
    parser = argparse.ArgumentParser(description='Train LASA-Unet')
    ...
    parser.add_argument('--weight-decay', type=float, default=5e-4) # Standard for SGD
...
def main():
    ...
    exp_name = f"{args.backbone}_{args.optimizer}_LASA{'_'.join(map(str, args.lasa_kernels))}_{args.dataset_name.replace('TSRS_RSNA-', '').lower()}"
    ...
    if args.test_only:
        ...
        if not os.path.exists(checkpoint_path):
            logging.error(f"FATAL: Checkpoint not found at {checkpoint_path}")
            return
    ...
    if args.optimizer == 'sgd':
        logging.info("Using SGD optimizer with momentum and Poly LR schedule.")
    ...
    max_iter = args.epochs * len(train_loader) # For Poly LR schedule
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_iterator):
            ...
            if args.optimizer == 'sgd':
                ...
                poly_lr_scheduler(optimizer, args.lr, current_iter, max_iter)
