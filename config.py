...
def download_and_extract_kaggle_dataset(dataset_id, target_dir):
    ...
    # KAGGLE_DATASET_MAPPING must be defined to find final_destination_folder_name
    # Importing it here to avoid circular dependency if datasets.py imports config
    from datasets import KAGGLE_DATASET_MAPPING as local_kaggle_dataset_mapping # Local import to avoid circular dependency
    for ds_name, ds_info in local_kaggle_dataset_mapping.items():
    ...
    # Heuristic for nested dataset structures (e.g., /kaggle/input/dataset_name/content/actual_dataset_folder)
...
# This is now defined here and imported locally in datasets.py to prevent circular imports
