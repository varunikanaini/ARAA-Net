...
# --- FINAL VERSION: MobileNetV2 + SE Blocks + Boundary Spatial Attention ---
...
from spatial_attn import BoundarySpatialAttention # <-- IMPORT THIS NEW MODULE
...
class Light_LASA_Unet(nn.Module):
    def __init__(self, num_classes=2, lasa_kernels=[1, 3, 5, 7]):
        ...
        # --- 2. DECODER with DEEP SUPERVISION (and Boundary Spatial Attention) ---
    ...
    def _decoder_block(self, in_channels, out_channels):
        # Each decoder block now includes the BoundarySpatialAttention module
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            BoundarySpatialAttention(kernel_size=3), # <-- BoundarySpatialAttention integrated here
    ...
    def forward(self, x):
        ...
        d4_out = self.decoder4(d4) # This now includes BoundarySpatialAttention
        ...
        d3_out = self.decoder3(d3) # This now includes BoundarySpatialAttention
        ...
        d2_out = self.decoder2(d2) # This now includes BoundarySpatialAttention
        ...
        d1_out = self.decoder1(d1) # This now includes BoundarySpatialAttention
