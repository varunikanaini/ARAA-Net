...
class Light_LASA_Unet(nn.Module):
    def __init__(self, num_classes=2, lasa_kernels=[1, 3, 5, 7]):
        ...
        # --- 2. DECODER with DEEP SUPERVISION (and CBAM) ---
    ...
    def _decoder_block(self, in_channels, out_channels):
        # Each decoder block now includes a CBAM module after the first convolution
    ...
    def forward(self, x):
        ...
        d4_out = self.decoder4(d4) # This now includes CBAM
        ...
        d3_out = self.decoder3(d3) # This now includes CBAM
        ...
        d2_out = self.decoder2(d2) # This now includes CBAM
        ...
        d1_out = self.decoder1(d1) # This now includes CBAM
        aux_outputs.append(F.interpolate(self.aux_conv_d1(d1_out), size=(input_h, input_w), mode='bilinear', align_corners=True))
        
        # Final output
        final_output = self.final_conv(d1_out) # CBAM is already applied in the last decoder block
