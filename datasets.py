...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, image_mask_paths, args, split='train'):
        ...
        # Initialize ColorJitter here, but apply it separately in __getitem__ for image only
        self.color_jitter_transform = transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1)

        # Define composed_transforms without ColorJitter (it will be applied separately)
        if self.split == 'train':
            self.composed_transforms = transforms.Compose([
                tr.RandomHorizontalFlip(),
                # Corrected: Assuming tr.FixedResize expects (height, width) as separate arguments
                tr.FixedResize(scale_h, scale_w),
                # Assuming tr.RandomCrop expects a (height, width) tuple
    ...
    def __getitem__(self, index):
        ...
        # Apply ColorJitter only to the image for training split, before joint transforms
        if self.split == 'train':
            img = self.color_jitter_transform(img)

        label = self.convert_label(target)

        # Ensure 'sample' dictionary is correctly formed for joint transforms
